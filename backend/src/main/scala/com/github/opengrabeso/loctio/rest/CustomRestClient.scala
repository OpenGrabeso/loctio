...
// adpted from io.udash.rest.SttpRestClient
object CustomRestClient {
  def defaultBackend(): SttpBackend[Id, Nothing] = HttpURLConnectionBackend()

  /**
    * Creates a client instance of some REST API trait which translates method calls into HTTP requests
    * to given URI using STTP.
    */
  @explicitGenerics def apply[RestApi: RawRest.AsRealRpc : RestMetadata](baseUri: String)(implicit backend: SttpBackend[Id, Nothing]): RestApi =
  ...
  /**
    ...
    */
  def asHandleRequest(baseUri: String)(implicit backend: SttpBackend[Id, Nothing]): RawRest.HandleRequest =
  ...
  private def asHandleRequest(baseUri: Uri)(implicit backend: SttpBackend[Id, Nothing]): RawRest.HandleRequest =
    RawRest.safeHandle(request => {
      val sttpReq = toSttpRequest(baseUri, request)
      callback =>
        // it should be possible to simplify this, but this is as close to the SttpRestClient code as possible
        Future.successful(sttpReq.send()).onCompleteNow(respTry => callback(respTry.map(fromSttpResponse)))
    })
}
