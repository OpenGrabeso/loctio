...
object TestStorage extends FileStore {
  case class FileItem(name: String, content: AnyRef, modified: java.util.Date, metadata: Map[String, String])
  ...
  def store(name: FileStore.FullName, obj: AnyRef, metadata: (String, String)*) = {
    ...
    files += name.name -> FileItem(name.name, obj, now, metadata.toMap)
  }
